...
class HelperTests(unittest.TestCase):
    ...
    def test_suggestions_route_to_fixed_provider_and_parse_bounded_results(self):
        for args, endpoint in (
            ...
            (["c++ & café", "ddg"], "https://suggestqueries.google.com/complete/search?client=firefox&hl=en&q="),
            (["c++ & café", "https://example.com"], "https://suggestqueries.google.com/complete/search?client=firefox&hl=en&q="),
        ):
            with self.subTest(args=args):
                opener = mock.MagicMock()
                response = opener.open.return_value.__enter__.return_value
                response.read.return_value = json.dumps([
                    args[0], [None, "  ", " first "] + ["x" * 1000] * 30
                ]).encode()
                buf = io.StringIO()
                ...
                self.assertEqual(request.full_url, endpoint + "c%2B%2B%20%26%20caf%C3%A9")
                self.assertEqual(opener.open.call_args.kwargs["timeout"], HELPER.SUGGEST_DEADLINE)
                response.read.assert_called_once_with(HELPER.SUGGEST_BYTES + 1)
                suggestions = json.loads(buf.getvalue())["suggestions"]
                self.assertEqual(suggestions[0], "first")
                self.assertEqual(len(suggestions), HELPER.SUGGEST_COUNT)
                self.assertTrue(all(len(s) <= HELPER.SUGGEST_CHARS for s in suggestions))

    def test_kagi_suggestion_failures_do_not_fall_back_to_google(self):
        for raw in (b"not json", b"{}", b'["query", []]', b"x" * (HELPER.SUGGEST_BYTES + 1)):
